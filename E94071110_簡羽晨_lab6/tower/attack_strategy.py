...
class AttackStrategy(ABC):
    """Abstract class of attack method (Default)"""
    @ abstractmethod
...
class SingleAttack(AttackStrategy):
    ...
    def attack(self, enemies, tower, cd_count):
        for en in enemies.get():
            if in_range(en, tower):                 #if any enemy is in the tower attacking range, then attack.
                en.health -= tower.get_damage()
                cd_count = 0
                return cd_count                     #After the attack, the function return and stop the for-loop
        return cd_count


class AOE(AttackStrategy):
    """attack every enemy in range once a time"""
    def attack(self, enemies, tower, cd_count):
        for en in enemies.get():                    # Keep attatcking enemies until the for-loop end
...
class Snipe(AttackStrategy):
    ...
    def attack(self, enemies, tower, cd_count):
        for en in enemies.get():
            if in_range(en, tower):
                enemies.retreat(en)         #self.__expedition.remove(enemy)
                cd_count = 0
                return cd_count
        return cd_count
