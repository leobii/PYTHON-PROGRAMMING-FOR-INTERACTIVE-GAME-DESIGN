...
class Tower:
    ...
    ''' 
    其實就如同類別內的其他函數一樣，利用self來指向自己，但在classmethod中的第一個參數，指向的自己是"該類別"的記憶體位置。
    classmethod的第一個參數和self一樣，也是隨你任意命名，不一定要叫”cls”。
    '''
    @classmethod
    def RapidTest(cls, x, y):
        rapid_test = cls(x, y, SingleAttack(), RAPID_TEST_IMAGE)
        print(rapid_test.level)
    ...
    def PCR(cls, x, y):
        ...
        pcr._cd_max_count = 120
        pcr._range = [100, 105, 110, 115, 120, 125]
        pcr._damage = [3.0, 3.2, 3.4, 3.6, 3.8, 4.0]        #pcr._range and pcr._damage will become useless if attack_strategy = Snipe()
        return pcr
